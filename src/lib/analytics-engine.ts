...
export async function calculateRetentionAnalytics() {
    ...
    const customerRetention: { [month: string]: { [months_later: number]: Set<string> } } = {};

    orders.forEach(order => {
        const email = order.customerInfo.customerEmail || order.customerIp; // Fallback a IP si no hay email
...
export async function calculateCustomerLTV() {
    ...
    ordersSnapshot.docs.forEach(doc => {
        const data = doc.data();
        const email = data.customerInfo.customerEmail || data.customerIp;
        ...
        if (metrics[email].orderCount === 1 || daysSince < metrics[email].lastOrderDays) {
